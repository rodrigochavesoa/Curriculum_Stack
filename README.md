...
Desenvolvedor Full-Stack Python especializado em **soluções AI-Driven**, com expertise em **arquitetura RAG**, **processamento de linguagem natural** e **automação de processos complexos**. Combino conhecimento profundo em UX Design com desenvolvimento backend robusto para criar produtos que unem inteligência artificial com experiência do usuário excepcional.

**Diferenciais:**
- Arquitetura de sistemas de IA com LLMs (Google Gemini, GPT)
- Pipeline completo RAG (Retrieval-Augmented Generation)
- Automação de processos governamentais e corporativos
- Design de produtos data-driven com foco em escalabilidade
...
`Google Gemini 2.5 Flash` • `LangChain Framework` • `RAG Architecture` • `FAISS Vector Database` • `Google Generative AI Embeddings` • `NLP` • `Semantic Search` • `Prompt Engineering`

### **Backend & Core Development**
`Python 3.8+` • `SQLite` • `PostgreSQL` • `RESTful APIs` • `Express.js` • `Node.js` • `Threading/AsyncIO` • `API Integration` • `Webhooks`

### **Data Engineering & Processing**
`Pandas` • `NumPy` • `PyMuPDF` • `Data Pipelines` • `CSV/JSON/Excel Processing` • `ETL Workflows` • `Document Processing`

### **Frontend & UI**
`Tkinter` • `Responsive Design` • `UX Research` • `Design Thinking` • `Prototyping` • `User-Centered Design`

### **DevOps & Tools**
`Git/GitHub` • `pytest` • `CI/CD` • `Environment Management` • `Code Coverage` • `Automated Versioning (setuptools-scm)`

### **APIs & Integrations**
`ClickUp API` • `PNCP API` • `Google APIs` • `Webhook Development` • `OAuth` • `Third-party Integration`

### **Geospatial & Data Science**
`GeoPy` • `Shapefile Processing` • `Spatial Analysis` • `Distance Calculations` • `Geographic Filtering`

---

## 💼 PROJETOS & EXPERIÊNCIA

### **🏋️ STAFF Personal Trainer** | *Founder & Lead Developer*
**Stack:** `Python` • `LangChain` • `Google Gemini 2.5` • `FAISS` • `RAG` • `Tkinter` • `SQLite` • `Pandas` • `PyMuPDF`

**Sistema desktop de IA para geração automatizada de fichas de treino personalizadas**

**Arquitetura Implementada:**
- 🤖 **Sistema RAG Completo:** Integração LangChain + FAISS para busca semântica em base científica vetorizada
- 🧠 **IA Generativa:** Implementação de prompt engineering avançado com Google Gemini 2.5 Flash
- 📚 **Knowledge Base:** Pipeline de processamento de PDFs científicos com chunking inteligente e embeddings
- 🗄️ **Database Design:** Schema relacional SQLite com migrações automáticas e histórico completo
- 🔐 **Sistema de Segurança:** Validação robusta de inputs, sanitização e regras automáticas baseadas em perfil
- 🎯 **Multi-tenancy:** Sistema de turmas com descoberta automática de datasets específicos
- 🧵 **Async Processing:** Threading para UI responsiva durante processamento de IA
- 📊 **Data Processing:** Manipulação complexa de DataFrames com Pandas para seleção inteligente

**Destaques Técnicos:**
- Arquitetura híbrida: triagem determinística + IA generativa contextualizada
- Sistema de feedback e evolução progressiva baseado em histórico
- Integração WhatsApp e export multiplataforma
- Interface gráfica profissional com tema customizado
- Logging detalhado para debugging e auditoria
- Validadores customizados com feedback visual em tempo real

**Resultados:**
- ✅ Automatização de 90%+ do processo de criação
- ✅ Fundamentação científica integrada via RAG
- ✅ Personalização inteligente com 7+ perfis especializados
- ✅ 12+ focos de treino com distribuição muscular otimizada

---

### **📋 Monitor de Licitações PNCP** | *Arquiteto & Desenvolvedor*
**Stack:** `Python` • `Node.js` • `Express.js` • `RESTful API` • `GeoPy` • `ClickUp API` • `Pandas` • `Shapefile Processing`

**Sistema escalável para monitoramento automatizado de licitações públicas em tempo real**

**Componentes Desenvolvidos:**
- 🔄 **API Integration:** Consumo da API PNCP com rate limiting e retry logic
- 🗺️ **Geospatial Engine:** Filtro geográfico por distância usando coordenadas e shapefiles do IBGE
- 🎯 **Sistema de Filtros Multi-nível:** 3 camadas (serviço, contexto, exclusões) configuráveis via `.env`
- 📊 **Export Engine:** Geração de relatórios em JSON, CSV e XLSX com dados consolidados
- 🔗 **Integração ClickUp:** Criação automática de tarefas via webhook e API REST
- ⏰ **Real-time Monitoring:** Consulta contínua com alertas para novas oportunidades
- 🛡️ **Error Handling:** Sistema robusto de tratamento de exceções e logs
...
- Pipeline ETL para processamento de dados geográficos do IBGE

**Casos de Uso Implementados:**
- Monitoramento de 50+ cidades em raio de 300km
- Alertas segmentados por setor de negócio
- Integração completa com task managers (ClickUp)
- Análise histórica e identificação de padrões
- Relatórios gerenciais automatizados

---

### **⚙️ WP7 Engenharia** | *Consultor de Produtividade & Automação*
...
- 🏗️ Automação de gestão de obras com integração multi-plataforma
- 📄 Sistema de gestão documental para licitações
- 💰 Módulos administrativos e financeiros customizados
- 📈 Dashboards de produtividade e KPIs
...
🎯 **Híbrido Tech + UX:** Combino desenvolvimento robusto com visão estratégica de produto  
🤖 **AI Specialist:** Expertise em implementação prática de LLMs e RAG architecture  
🔧 **Problem Solver:** Histórico de automação de processos complexos e críticos  
📊 **Data-Driven:** Decisões baseadas em dados, métricas e evidências  
🚀 **Fast Learner:** Capacidade de aprender e implementar novas tecnologias rapidamente  

---

## 🎯 OBJETIVO PROFISSIONAL

Busco oportunidades em empresas inovadoras que valorizam **IA aplicada**, **automação inteligente** e **produtos centrados no usuário**. Interesse especial em:

- **AI/ML Engineer** em projetos de IA Generativa e RAG
- **Senior Python Developer** com foco em backend e automação
...
> *"Transformo problemas complexos em soluções elegantes usando IA, código limpo e empatia pelo usuário."*
